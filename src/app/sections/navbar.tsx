...
export default function Navbar() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            if (ignoreScroll.current) return; // Ignore scroll during lang switch
    ...
    /* SHOW NAVBAR WHEN MOUSE HOVERS TOP */
    useEffect(() => {
    ...
    /* FIX: Navbar hiding when switching languages */
    useEffect(() => {
        setShow(true); // Forzar visible

        ignoreScroll.current = true;

        // Esperar el reflow del DOM
        const timeout = setTimeout(() => {
            ignoreScroll.current = false;
            lastScroll.current = window.scrollY; // Reset scroll base
        }, 300);
            ...
            <div className="mt-6 flex">
                ...
                >
                    ...
                    <div className="flex justify-around w-1/3 font-MPMomo">
                        <div
                            onClick={() =>
                                window.open(navlist[0].content1[1], "_blank")
                            }
                        ...
                        <div
                            onClick={() =>
                                window.open(navlist[0].content2[1], "_blank")
                            }
                            className="text-lg font-semibold hover:text-emerald-500 cursor-pointer"
                        >
                        ...
                        <div className="text-lg font-semibold hover:text-emerald-500 cursor-pointer">
                            {navlist[0].content3}
                        </div>
                    </div>
                </div>
